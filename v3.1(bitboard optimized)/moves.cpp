...
void make_random_move() {
	...
	while (limit--) {
		...
		if(is_move_valid(start, end, engine)) {
			...
			// Make the move
			make_move(start_pos, end_pos, engine);

			// If it chess after move, undo the move
			if(is_in_check(engine)) {
				board = tmp.board;
				en_passant[0] = tmp.en_passant[0];
				en_passant[1] = tmp.en_passant[1];
				castling[0] = tmp.castling[0];
				castling[1] = tmp.castling[1];
				continue;
			}
