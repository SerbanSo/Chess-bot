...
bool is_attacked(int board[2][9][9], position pieces_positions[2][16], int pieces_number[2], int king_moved[2], int left_rook_moved[2], int right_rook_moved[2], int en_passant_available[2][9], position king_position[2], int x, int y, int player)
{
	...
	for ( k = 0; k < pieces_number[1 - player]; k++ )
	{
		i = pieces_positions[1 - player][k].x;
		j = pieces_positions[1 - player][k].y;

		if ( board[1 - player][i][j] == PAWN )
		{
			if ( y == j + direction )
			{
				if ( x == i + 1 || x == i - 1 )
		...
		else if ( board[1 - player][i][j] == KNIGHT )
		{
			attacked = knight_valid_move(board, i, j, x, y);
		}
		else if ( board[1 - player][i][j] == BISHOP )
		{
			attacked = bishop_valid_move(board, i, j, x, y);
		}
		else if ( board[1 - player][i][j] == ROOK )
		{
			attacked = rook_valid_move(board, i, j, x, y);
		}
		else if ( board[1 - player][i][j] == QUEEN )
		{
			attacked = queen_valid_move(board, i, j, x, y);
		}
		else if ( board[1 - player][i][j] == KING )
		{
			attacked = king_valid_move(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, i, j, x, y, 1 - player);
		}
...
bool is_in_check_after_move(int board[2][9][9], position pieces_positions[2][16], int pieces_number[2], int king_moved[2], int left_rook_moved[2], int right_rook_moved[2], int en_passant_available[2][9], position king_position[2], int startX, int startY, int endX, int endY, int player)
{
	...
	if ( swap_aux1 == KING )
	{
		result_aux = is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, endX, endY, player);
	}
	else //the king is in check after moving the piece
	{
		result_aux = is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, king_position[player].x, king_position[player].y, player);
	}
...
{
	//starting square cannot be identical to end square and has to be within the board
	if ( ( move[2] < 'a' || move[2] > 'h' ) || ( move[3] < '1' || move[3] > '8' ) || ( move[0] == move[2] && move[1] == move[3] ) )
	{
		return false;
	}
	...
	if ( board[player][endX][endY] != EMPTY || piece == EMPTY || is_in_check_after_move(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, startX, startY, endX, endY, player) )
	{
		return false;
	}
	else if ( piece == PAWN )
	{
		return pawn_valid_move(board, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, startX, startY, endX, endY, player);
	}
	else if ( piece == KNIGHT)
	{
		return knight_valid_move(board, startX, startY, endX, endY);
	}
	else if ( piece == BISHOP )
	{
		return bishop_valid_move(board, startX, startY, endX, endY);
	}
	else if ( piece == ROOK )
	{
		return rook_valid_move(board, startX, startY, endX, endY);
	}
	else if ( piece == QUEEN)
	{
		return queen_valid_move(board, startX, startY, endX, endY);
	}
	else if ( piece == KING)
	{
		return king_valid_move(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, startX, startY, endX, endY, player);
	}
...
void change_position(position pieces_positions[2][16], int pieces_number[2], int player, int startX, int startY, int endX, int endY)
{
	...
	{
		if ( pieces_positions[player][i].x == startX && pieces_positions[player][i].y == startY )
		{
			pieces_positions[player][i].x = endX;
			pieces_positions[player][i].y = endY;
...
void delete_piece(position pieces_positions[2][16], int pieces_number[2], int player, int startX, int startY)
{
	for ( int i = 0; i < pieces_number[player]; i++ )
	{
		if ( pieces_positions[player][i].x == startX && pieces_positions[player][i].y == startY )
		{
			for ( int j = i; j < pieces_number[player]; j++ )
			{
				pieces_positions[player][j].x = pieces_positions[player][j + 1].x;
				pieces_positions[player][j].y = pieces_positions[player][j + 1].y;
			}

			pieces_number[player]--;
...
void make_move(int board[2][9][9], position pieces_positions[2][16], int pieces_number[2], int king_moved[2], int left_rook_moved[2], int right_rook_moved[2], int en_passant_available[2][9], position king_position[2], char *move, int player)
{
	...
	if ( board[player][startX][startY] == KING )
	{
		...
		if ( endY == base_line && king_moved[player] == 0 )
		{
			if ( endX == C ) //left castle
			{
				...
				change_position(pieces_positions, pieces_number, player, startX, startY, endX, endY);
				change_position(pieces_positions, pieces_number, player, A, base_line, endX + 1, endY);
			...
			else if ( endX == G ) //right castle
			{
				...
				change_position(pieces_positions, pieces_number, player, startX, startY, endX, endY);
				change_position(pieces_positions, pieces_number, player, H, base_line, endX - 1, endY);
				board[player][endX][endY] = KING;
				board[player][startX][startY] = EMPTY;
				board[player][endX - 1][endY] = ROOK;
	...
	if ( board[player][startX][startY] == PAWN )
	{
		...
		{
			change_position(pieces_positions, pieces_number, player, startX, startY, endX, endY);
			delete_piece(pieces_positions, pieces_number, 1 - player, endX, startY);
	...
	{
		delete_piece(pieces_positions, pieces_number, 1 - player, endX, endY);
	}
...
// generates up to 100,000 random moves until it find a valid one or resigns
void make_random_move()
{
	...
	while ( limit-- )
	{
		...
		if ( board[engine][r1 + 1][r2 + 1] != EMPTY )
		{
			r3 = rand() % 8;
			r4 = rand() % 8;
			...
			if ( is_move_valid(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, move, engine) )
			{
				// Updates the engine's matrices
				make_move(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, move, engine);
...
bool rook_valid_move(int board[2][9][9], int startX, int startY, int endX, int endY)
{
	...
	if ( endX == startX ) //vertical move
	{
		...
		// For each square, check for collision with either a friendly or enemy piece
...
bool queen_valid_move(int board[2][9][9], int startX, int startY, int endX, int endY)
{
	...
	if ( !(bishop_valid_move(board, startX, startY, endX, endY) || rook_valid_move(board, startX, startY, endX, endY)) )
	{
		return false;
	}
...
bool king_valid_move(int board[2][9][9], position pieces_positions[2][16], int pieces_number[2], int king_moved[2], int left_rook_moved[2], int right_rook_moved[2], int en_passant_available[2][9], position king_position[2], int startX, int startY, int endX, int endY, int player)
{
	...
	if ( startX == E && startY == base_line && king_moved[player] == 0 )
	{
		...
		{
			return !(is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, E, base_line, player)
					|| is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, D, base_line, player)
					|| is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, C, base_line, player));
		}
		...
		{
			return !(is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, E, base_line, player)
					|| is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, F, base_line, player)
					|| is_attacked(board, pieces_positions, pieces_number, king_moved, left_rook_moved, right_rook_moved, en_passant_available, king_position, G, base_line, player));
		}
	}
